...
When the server is ready, open Power+ and go to Settings -> About -> Show Advanced Options -> Debug mode and type in the port the server is running on (the default is 2750). Then, reload Power+ and go to Settings -> Prerelease testing and select "Debugging". This will make Power+ load from the local server instead of the live version the next time you reload.
