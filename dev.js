...
<div onclick="dtps.selectedClass = 'dash';" class="class masterStream ` + streamClass + `">
  ...
  $( ".class:not(.google)" ).click(function(event) {
	  ...
	  if (dtps.fadedColors) prev = "linear-gradient(to bottom right, " + window.getComputedStyle(document.getElementsByClassName("background")[0]).getPropertyValue($("body").hasClass("dark") ? ($("body").hasClass("midnight") ? "--dark" : "--norm") : "--light") + ", " + jQuery("body").css("background-color") + ")"
	  $(".btn.google").hide();
...
<div class="abtpage display">
    ...
    <br /><br />
    <div onclick="dtps.fullNames = !dtps.fullNames; localStorage.setItem('dtpsFullNames', dtps.fullNames); dtps.showClasses(true);" class="switch sudo` + (window.localStorage.dtpsFullNames == "true" ? " active" : "") + `"><span class="head"></span></div>
